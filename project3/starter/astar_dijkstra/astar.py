...
def astar(map_data, start, goal):
    ...
    ## Enter code here
    def h(cell):
        return np.linalg.norm(celltoworld(leftbound, resolution, cell) - goal)
    ...
    dist[start_node] = 0
    pq.append((h(start_cell), 0, start_node))  # (distance, node)
    final_cost = 0
    completed = False
    while pq:
        current_dist_plus_euclidean, current_dist, current_node = heapq.heappop(pq)  # Get node with smallest distance
        ...
        for neighbor_node in valid_neighbors:
            ...
            if alt_dist < dist[neighbor_node]:
                ...
                heapq.heappush(pq, (alt_dist + h(numbertocell([mx, my], neighbor_node)), alt_dist, neighbor_node)) # A* uses f(n) = g(n) + h(n)
